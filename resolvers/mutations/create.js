...
const createAttachment = async (_, { attachment }, { user }) => {
  ...
  try {
    // Upload Attachment
    const { public_id, secure_url, format } = await cloudinary.uploader.upload(
      url,
      {
        resource_type: "auto",
        access_mode: "public",
...
const createContactPage = async (_, { contactPage }, { user }) => {
  // authentication
  if (!user) {
    throw new Error("User not logged In");
  }
  ...
  try {
    const { site, site_name, favicon } = contactPage;
    let image = null;

    // if favicon is available
    if (favicon) {
      const { url, filename } = favicon;
    ...
    const { rows } = await db.query(
      `
		INSERT INTO contact_us_pages (site, site_name, owner, favicon)
		VALUES ($1, $2, $3, $4)
		RETURNING *
		`,
      [site, site_name, user.id, image]
    );

    return rows[0];
  } catch (error) {
    console.log(error.message);
    throw new Error("Failed creating contact page: " + error.message);
  }
};
